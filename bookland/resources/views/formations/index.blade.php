...
<style>
    ...
    /* (For brevity, we assume it's included – but you must copy the full CSS) */
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-zn-danger-ghost:hover { background: #fddde2; color: var(--rose); }
...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <a href="#">Gestion</a>
        <span class="zn-bc-sep">›</span>
        <span class="zn-bc-cur">Formations</span>
    </div>
    ...
    <div class="zn-header">
        <div class="zn-header-left">
            <h1>Formations</h1>
            <p>Gestion des demandes de formation</p>
        </div>
        @if(auth()->user()->role === 'delegue')
            <div class="zn-header-actions">
                <a href="{{ route('formations.create') }}" class="btn-zn btn-zn-primary">
    ...
    <div class="zn-search-bar">
        <form method="GET" action="{{ route('formations.index') }}" style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: flex-end;">
            ...
            <div class="filter-actions">
                ...
                <a href="{{ route('formations.index') }}" class="btn-zn btn-zn-sm btn-zn-danger-ghost">
    ...
    <div class="zn-card">
        <div class="zn-card-header">
            ...
            <span class="zn-count">{{ $formations->total() }} demande(s)</span>
        </div>
        ...
        <div style="overflow-x: auto;">
            <table class="zn-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><strong>{{ $f->compte->etablissement }}</strong></td>
                        <td>{{ $f->type }}</td>
                        <td>
                            @foreach($f->date_demande as $date)
                        ...
                        <td>
                            ...
                            @endphp
                            <span class="dr-badge {{ $badgeClass }}">{{ $statuts[$f->statut] }}</span>
                        </td>
                        <td>
                            <div class="actions-cell">
                                <a href="{{ route('formations.show', $f) }}" class="btn-zn btn-zn-sm btn-zn-info">
                                    <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                @if(in_array(auth()->user()->role, ['admin','rbo']) || (auth()->user()->role === 'delegue' && $f->delegue_id === auth()->id()))
                                    ...
                                    <form action="{{ route('formations.destroy', $f) }}" method="POST" style="display:inline;" onsubmit="return confirm('Supprimer cette formation ?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="5">
                                <div class="zn-empty">
                                    ...
                                    <h3>Aucune demande de formation</h3>
                                    <p>{{ request('compte_id') || request('statut') || request('type') || request('delegue_id') ? 'Aucun résultat pour les filtres sélectionnés.' : 'Commencez par créer une nouvelle demande.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if($formations->hasPages())
            <div class="zn-pagination">
                {{ $formations->withQueryString()->links('vendor.pagination.custom') }}
            </div>
        @endif
    </div>
</div>
