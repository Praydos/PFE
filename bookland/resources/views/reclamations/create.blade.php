    ...
    <style>
        ...
        *::after {
        ...
        .ac-select.err {
            ...
            box-shadow: 0 0 0 3px rgba(232, 80, 106, .1);
        }
...
@section('content')
    <div class="ac-page">
        ...
        <div class="ac-header">
            ...
            <p>{{ isset($reclamation) ? 'Mettez à jour les informations de la réclamation.' : 'Enregistrez une nouvelle réclamation client.' }}
            </p>
        </div>

        @if($errors->any())
            <div
                style="display:flex; align-items:flex-start; gap:.75rem; padding:1rem 1.25rem; border-radius:var(--r3); background:var(--rose-l); border:1px solid rgba(232,80,106,.25); margin-bottom:1.5rem;">
                <svg width="16" height="16" fill="none" stroke="var(--rose)" stroke-width="2" viewBox="0 0 24 24">
                    <circle cx="12" cy="12" r="10" />
                    <line x1="12" y1="8" x2="12" y2="12" />
                    <line x1="12" y1="16" x2="12.01" y2="16" />
                </svg>
                <div><strong style="display:block;margin-bottom:.3rem;">Veuillez corriger les erreurs suivantes&nbsp;:</strong>
                    <ul style="padding-left:1.2rem;">@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul>
                </div>
            </div>
        @endif
            ...
            <form method="POST" action="{{ $action }}">
                ...
                <div class="ac-card-body">
                    ...
                    <div class="ac-row ac-row-2">
                        <div class="ac-group">
                            ...
                            <div class="ac-sel-wrap">
                                <select name="compte_id" id="compte_id" class="ac-select @error('compte_id') err @enderror"
                                    required>
                        ...
                        <div class="ac-group">
                            ...
                            <div class="ac-sel-wrap">
                                <select name="contact_id" id="contact_id"
                                    class="ac-select @error('contact_id') err @enderror" required>
                    ...
                    <div class="ac-row ac-row-3">
                        <div class="ac-group">
                            <label class="ac-label" for="date_reclamation">Date réclamation <span
                                    class="req">*</span></label>
                            <input type="date" name="date_reclamation" id="date_reclamation"
                                class="ac-input @error('date_reclamation') err @enderror"
                                value="{{ old('date_reclamation', $isEdit ? $reclamation->date_reclamation->format('Y-m-d') : now()->format('Y-m-d')) }}"
                                required>
                    ...
                    <div class="ac-row ac-row-2">
                        <div class="ac-group">
                            ...
                            <div class="ac-sel-wrap">
                                <select name="categorie" id="categorie" class="ac-select @error('categorie') err @enderror"
                                    required>
                                    <option value="">-- Sélectionnez --</option>
                        ...
                        <div class="ac-group">
                            ...
                            <div class="ac-sel-wrap">
                                <select name="sous_categorie" id="sous_categorie"
                                    class="ac-select @error('sous_categorie') err @enderror">
                                    <option value="">-- Sélectionnez --</option>
                                </select>
                            </div>
                        ...
                        <div class="ac-sel-wrap">
                            <select name="produit_id" id="produit_id" class="ac-select">
                                ...
                                @foreach($produits as $p)
                                    <option value="{{ $p->id }}" {{ old('produit_id', $isEdit ? $reclamation->produit_id : '') == $p->id ? 'selected' : '' }}>{{ $p->titre }} ({{ $p->isbn_13 ?? $p->isbn_10 }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <input type="hidden" name="module_lie" id="module_lie"
                        value="{{ old('module_lie', $isEdit ? $reclamation->module_lie : '') }}">

                    <div class="ac-group">
                        <label class="ac-label" for="description">Description <span class="req">*</span></label>
                        <textarea name="description" id="description" rows="3"
                            class="ac-textarea @error('description') err @enderror"
                            required>{{ old('description', $isEdit ? $reclamation->description : '') }}</textarea>
                    ...
                    @if($isEdit)
                        ...
                        <div class="ac-row ac-row-2">
                            <div class="ac-group">
                                ...
                                <textarea name="analyse" id="analyse" rows="2"
                                    class="ac-textarea">{{ old('analyse', $reclamation->analyse) }}</textarea>
                            </div>
                            <div class="ac-group">
                                <label class="ac-label" for="reponse">Réponse</label>
                                <textarea name="reponse" id="reponse" rows="2"
                                    class="ac-textarea">{{ old('reponse', $reclamation->reponse) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="ac-row ac-row-3">
                            <div class="ac-group">
                                ...
                                <input type="date" name="date_reponse" id="date_reponse" class="ac-input"
                                    value="{{ old('date_reponse', $reclamation->date_reponse ? $reclamation->date_reponse->format('Y-m-d') : '') }}">
                            </div>
                            <div class="ac-group">
                        ...
                        <div class="ac-row ac-row-4">
                            <div class="ac-group">
                                ...
                                <input type="date" name="date_cloture" id="date_cloture" class="ac-input"
                                    value="{{ old('date_cloture', $reclamation->date_cloture ? $reclamation->date_cloture->format('Y-m-d') : '') }}">
                            </div>
                            <div class="ac-group">
                                <label class="ac-check-row"
                                    style="padding:.7rem 1rem;background:var(--subtle);border:1px solid var(--border);border-radius:var(--r2);">
    ...
    <script>
        ...
        const currentResponsableId = @json(old('responsable_id', $isEdit ? $reclamation->responsable_id : ''));

        function loadContacts() {
            const compteId = compteSelect.value;
            const specimenSelect = document.getElementById('specimen_id');
            ...
            if (specimenSelect) {
                fetch(`/api/comptes/${compteId}/bss`)
                    ...
                    .then(data => {
                        ...
                        @if($isEdit)
                            const currentSpecimen = '{{ old("specimen_id", $reclamation->specimen_id) }}';
                            if (currentSpecimen) specimenSelect.value = currentSpecimen;
                        @endif
                    });
            }
        }
