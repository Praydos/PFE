...
<style>
    /* ===== FULL DESIGN SYSTEM CSS (same as BSS index, non-conformites index) ===== */
...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <span class="zn-bc-cur">Réclamations</span>
    </div>
    ...
    {{-- Success message (if any, from session) --}}
    @if(session('success'))
    ...
    <div class="zn-card">
        <div class="zn-card-header">
            ...
            <span class="zn-count">{{ $reclamations->total() }} réclamation(s)</span>
        </div>
            ...
            <table class="zn-table">
                ...
                <tbody>
                    @forelse($reclamations as $r)
                        <tr>
                            <td><span style="font-family:var(--font-mono);">{{ $r->reference }}</span></td>
                            <td>{{ $r->compte->etablissement }}</td>
                            <td>{{ $r->categorie }} @if($r->sous_categorie) <span style="color:var(--text-muted);">({{ $r->sous_categorie }})</span> @endif</td>
                            <td>{{ $r->date_reclamation->format('d/m/Y') }}</td>
                            <td>
                                @php
                                    $badgeClass = 'bd-none';
                                    if ($r->statut === 'brouillon') $badgeClass = 'bd-amber';
                                    elseif ($r->statut === 'en_cours') $badgeClass = 'bd-blue';
                                    elseif ($r->statut === 'resolu') $badgeClass = 'bd-green';
                                    elseif ($r->statut === 'ferme') $badgeClass = 'bd-none';
                                @endphp
                                <span class="dr-badge {{ $badgeClass }}">{{ ucfirst($r->statut) }}</span>
                            </td>
                            <td class="actions-cell">
                                <a href="{{ route('reclamations.show', $r) }}" class="btn-zn btn-zn-sm btn-zn-info">
                        ...
                        <tr>
                            <td colspan="6">
                                <div class="zn-empty">
                                    ...
                                    <p>{{ request('statut') || request('categorie') || request('compte_id') ? 'Aucun résultat pour les filtres sélectionnés.' : 'Commencez par créer une nouvelle réclamation.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
