...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <span class="zn-bc-sep">›</span>
        @if(isset($annees_scolaire))
            <a href="{{ route('annees-scolaires.show', $annees_scolaire) }}">{{ $annees_scolaire->libelle }}</a>
            <span class="zn-bc-sep">›</span>
            <span class="zn-bc-cur">Modifier</span>
        @else
            <span class="zn-bc-cur">Nouvelle année</span>
        @endif
    </div>
    ...
    <div class="zn-header">
        ...
        @else
            ...
            <p>Remplissez les informations ci‑dessous pour ajouter une nouvelle année scolaire.</p>
        @endif
    </div>
    ...
    <div class="fp-card">
        <form method="POST" action="{{ isset($annees_scolaire) ? route('annees-scolaires.update', $annees_scolaire) : route('annees-scolaires.store') }}" novalidate>
            @csrf
            @if(isset($annees_scolaire)) @method('PUT') @endif
            ...
            <div class="fp-section">
                <div class="fp-section-head">
                    ...
                    <div class="fp-section-meta">
                        <div class="fp-section-title">Informations de l'année scolaire</div>
                        <div class="fp-section-sub">Libellé et période</div>
                    </div>
                </div>

                {{-- First row: Libellé + Date début --}}
                <div class="fp-row fp-row-2">
                    <div class="frm-group">
                        <label class="frm-label" for="libelle">Libellé <span class="req">*</span></label>
                        <div class="frm-input-wrap">
                            ...
                            <input type="text" id="libelle" name="libelle"
                                   class="frm-input @error('libelle') is-invalid @enderror"
                                   value="{{ old('libelle', $annees_scolaire->libelle ?? '') }}"
                                   required autocomplete="off">
                        </div>
                        @error('libelle')<span class="frm-error">{{ $message }}</span>@enderror
                    </div>

                    <div class="frm-group">
                        <label class="frm-label" for="date_debut">Date début <span class="req">*</span></label>
                        <div class="frm-input-wrap">
                            <span class="frm-icon">
                                <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/>
                            ...
                            <input type="date" id="date_debut" name="date_debut"
                                   class="frm-input @error('date_debut') is-invalid @enderror"
                                   value="{{ old('date_debut', $annees_scolaire->date_debut ?? '') }}"
                                   required>
                        </div>
                        @error('date_debut')<span class="frm-error">{{ $message }}</span>@enderror
                    </div>
                </div>

                {{-- Second row: Date fin (full width) --}}
                <div class="fp-row">
                    <div class="frm-group">
                        <label class="frm-label" for="date_fin">Date fin <span class="req">*</span></label>
                        <div class="frm-input-wrap">
                            <span class="frm-icon">
                                <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/>
                            ...
                            <input type="date" id="date_fin" name="date_fin"
                                   class="frm-input @error('date_fin') is-invalid @enderror"
                                   value="{{ old('date_fin', $annees_scolaire->date_fin ?? '') }}"
                                   required>
                        </div>
            ...
            <div class="fp-footer">
                ...
                <a href="{{ route('annees-scolaires.index') }}" class="btn-zn btn-zn-ghost">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <button type="submit" class="btn-zn btn-zn-primary">
                    ...
                    {{ isset($annees_scolaire) ? 'Mettre à jour' : 'Créer' }}
                </button>
            </div>
        </form>
    </div>
