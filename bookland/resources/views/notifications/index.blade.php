...
@section('content')
    <div class="nc-page" x-data="notificationCenter()">
        ...
        <div class="nc-header">
            ...
            <button class="nc-mark-all-btn" x-show="unreadCount > 0" @click="markAllAsRead">
        ...
        <div class="nc-filters-row">
            ...
            <div class="nc-filter-group">
                <button class="nc-pill" :class="filters.is_read === '' ? 'active' : ''"
                    @click="setFilter('is_read', '')">Tous</button>
                <button class="nc-pill" :class="filters.is_read === 'false' ? 'active' : ''"
                    @click="setFilter('is_read', 'false')">
                    <span class="nc-pill-dot unread-dot" aria-hidden="true"></span>Non lus
                </button>
                <button class="nc-pill" :class="filters.is_read === 'true' ? 'active' : ''"
            ...
            <div class="nc-filter-group">
                <template x-for="t in typeOptions" :key="t.value">
                    <button class="nc-pill" :class="filters.type === t.value ? 'active type-' + t.value : ''"
            ...
            {{-- Category --}}
            <div class="nc-filter-group">
                <select class="nc-select" x-model="filters.category" @change="fetchNotifications(1)"
                ...
                <div>
                    ...
                    <div class="nc-group-list">
                        <template x-for="n in todayNotifications" :key="n.id">
                            <div class="nc-item" :class="[!n.is_read ? 'unread' : 'read', 'type-border-' + n.type]"
                                @click="!n.is_read && markAsRead(n.id)" :tabindex="!n.is_read ? 0 : -1"
                                :aria-label="n.title + (!n.is_read ? ' — non lu' : '')" role="listitem">
        ...
        <div class="nc-pagination" x-show="pagination.last_page > 1">
            <span class="nc-page-info" x-text="`${pagination.from || 0}–${pagination.to || 0} sur ${pagination.total}`">
            </span>
            <div class="nc-page-btns">
                <button class="nc-page-btn" :disabled="pagination.current_page === 1"
                ...
                <button class="nc-page-btn" :disabled="pagination.current_page === pagination.last_page"
                    @click="fetchNotifications(pagination.current_page + 1)">
                    Suiv.
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
                        stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
...
@push('styles')
    ...
    ───────────────────────────────────────────── */
        :root {
            --nc-page-bg: transparent;
            --nc-surface: var(--bg-card);
            --nc-surface-hover: var(--bg-hover);
            --nc-border: var(--border);
            --nc-border-md: var(--border-md);
            --nc-text-primary: var(--text-primary);
            --nc-text-secondary: var(--text-secondary);
            --nc-text-muted: var(--text-muted);
            --nc-radius-sm: var(--r-xs);
            --nc-radius-md: var(--r-sm);
            --nc-radius-lg: var(--r-md);

            /* type palette */
            --nc-success-bg: var(--green-light);
            --nc-success-fg: var(--green);
            --nc-success-acc: var(--green);

            --nc-reminder-bg: var(--amber-light);
            --nc-reminder-fg: var(--amber);
            --nc-reminder-acc: var(--amber);

            --nc-deadline-bg: var(--rose-light);
            --nc-deadline-fg: var(--rose);
            --nc-deadline-acc: var(--rose);

            --nc-system-bg: var(--blue-light);
            --nc-system-fg: var(--blue);
            --nc-system-acc: var(--blue);
        }
    ...
    ───────────────────────────────────────────── */
        ...
        .nc-pill.active.type-success {
            background: var(--nc-success-acc);
            border-color: var(--nc-success-acc);
            color: #fff;
        }

        .nc-pill.active.type-reminder {
            background: var(--nc-reminder-acc);
            border-color: var(--nc-reminder-acc);
            color: #fff;
        }

        .nc-pill.active.type-deadline {
            background: var(--nc-deadline-acc);
            border-color: var(--nc-deadline-acc);
            color: #fff;
        }

        .nc-pill.active.type-system {
            background: var(--nc-system-acc);
            border-color: var(--nc-system-acc);
            color: #fff;
        }
    ...
    ───────────────────────────────────────────── */
        ...
        .nc-item.type-border-success::before {
            background: var(--nc-success-acc);
        }

        .nc-item.type-border-reminder::before {
            background: var(--nc-reminder-acc);
        }

        .nc-item.type-border-deadline::before {
            background: var(--nc-deadline-acc);
        }

        .nc-item.type-border-system::before {
            background: var(--nc-system-acc);
        }
        ...
        .icon-success {
            ...
            color: var(--nc-success-fg);
        }

        .icon-reminder {
            background: var(--nc-reminder-bg);
            color: var(--nc-reminder-fg);
        }

        .icon-deadline {
            background: var(--nc-deadline-bg);
            color: var(--nc-deadline-fg);
        }

        .icon-system {
            background: var(--nc-system-bg);
            color: var(--nc-system-fg);
        }
