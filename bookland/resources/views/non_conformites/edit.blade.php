...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <span class="zn-bc-sep">›</span>
        <a href="{{ route('non-conformites.show', $non_conformite) }}">{{ $non_conformite->numero }}</a>
        <span class="zn-bc-sep">›</span>
        <span class="zn-bc-cur">Modifier</span>
    </div>

    <div class="zn-header">
        <h1>Modifier la non‑conformité #{{ $non_conformite->numero }}</h1>
    ...
    <div class="fp-card">
        <form method="POST" action="{{ route('non-conformites.update', $non_conformite) }}">
            @csrf
            @method('PUT')
            ...
            <div class="fp-section">
                <div class="fp-section-head">
                    ...
                    <div class="fp-section-meta">
                        <div class="fp-section-title">Informations de la non‑conformité</div>
                        <div class="fp-section-sub">Champs principaux et statut</div>
                    </div>
                </div>

                {{-- Compte et contact (2 colonnes) --}}
                        ...
                        <div class="frm-select-wrap">
                            <select name="compte_id" id="compte_id" class="frm-select @error('compte_id') is-invalid @enderror" required>
                                ...
                                @foreach($comptes as $c)
                                    <option value="{{ $c->id }}" {{ old('compte_id', $non_conformite->compte_id) == $c->id ? 'selected' : '' }}>
                                        {{ $c->etablissement }}
                                    </option>
                                @endforeach
                            </select>
                ...
                {{-- Catégorie et sous‑catégorie (2 colonnes) --}}
                <div class="fp-row fp-row-2">
                    <div class="frm-group">
                        ...
                        <div class="frm-select-wrap">
                            <select name="categorie" id="categorie" class="frm-select @error('categorie') is-invalid @enderror" required>
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat }}" {{ old('categorie', $non_conformite->categorie) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="frm-group">
                        ...
                        <div class="frm-select-wrap">
                            <select name="sous_categorie" id="sous_categorie" class="frm-select @error('sous_categorie') is-invalid @enderror">
                                <option value="">— Sélectionnez —</option>
                                {{-- Options will be filled dynamically via JavaScript based on selected category --}}
                            </select>
                        </div>
                ...
                <div class="frm-group">
                    ...
                    <input type="text" name="objet" id="objet"
                           ...
                           value="{{ old('objet', $non_conformite->objet) }}" required>
                ...
                <div class="frm-group">
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="frm-textarea @error('description') is-invalid @enderror">{{ old('description', $non_conformite->description) }}</textarea>
                    @error('description')<span class="frm-error">{{ $message }}</span>@enderror
                </div>

                {{-- Date NC et Statut (2 colonnes) --}}
                <div class="fp-row fp-row-2">
                    <div class="frm-group">
                        <label class="frm-label" for="date_nc">Date NC <span class="req">*</span></label>
                        <input type="date" name="date_nc" id="date_nc"
                               class="frm-input @error('date_nc') is-invalid @enderror"
                               value="{{ old('date_nc', $non_conformite->date_nc->format('Y-m-d')) }}" required>
                        @error('date_nc')<span class="frm-error">{{ $message }}</span>@enderror
                    </div>

                    <div class="frm-group">
                        <label class="frm-label" for="statut">Statut <span class="req">*</span></label>
                        <div class="frm-select-wrap">
                            <select name="statut" id="statut" class="frm-select @error('statut') is-invalid @enderror" required>
                ...
                <div class="frm-group">
                    ...
                    <input type="date" name="date_cloture" id="date_cloture"
                           ...
                           value="{{ old('date_cloture', $non_conformite->date_cloture ? $non_conformite->date_cloture->format('Y-m-d') : '') }}">
            ...
            <div class="fp-footer">
                ...
                <a href="{{ route('non-conformites.show', $non_conformite) }}" class="btn-zn btn-zn-ghost">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
<script>
    // Dynamic contact loading based on compte selection
    const compteSelect = document.getElementById('compte_id');
    const contactSelect = document.getElementById('contact_id');
    const currentContactId = '{{ old('contact_id', $non_conformite->contact_id) }}';
    ...
    function loadContacts() {
        ...
        fetch(`/api/comptes/${compteId}/contacts`)
            ...
            .catch(() => contactSelect.innerHTML = '<option value="">Erreur de chargement</option>');
    }

    compteSelect.addEventListener('change', loadContacts);
    if (compteSelect.value) loadContacts();

    // Dynamic sous‑catégorie based on catégorie selection
    const categorieSelect = document.getElementById('categorie');
    const sousCategorieSelect = document.getElementById('sous_categorie');
    const oldSousCategorie = '{{ old('sous_categorie', $non_conformite->sous_categorie) }}';

    // Map of category => array of sous-categories
    const sousCategoriesMap = {
        'produit': @json($sousCategoriesProduit ?? []),
        'specimen': @json($sousCategoriesSpecimen ?? []),
        // Add other categories as needed (examen, formation, etc.)
    };

    function updateSousCategories() {
        const category = categorieSelect.value;
        const options = sousCategoriesMap[category] || [];
        let html = '<option value="">— Sélectionnez —</option>';
        options.forEach(sc => {
            html += `<option value="${sc}" ${oldSousCategorie == sc ? 'selected' : ''}>${sc}</option>`;
        });
        sousCategorieSelect.innerHTML = html;
    }

    categorieSelect.addEventListener('change', updateSousCategories);
    updateSousCategories(); // initialize
</script>
@endsection