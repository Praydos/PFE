...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <a href="#">Gestion</a>
        <span class="zn-bc-sep">›</span>
        <span class="zn-bc-cur">Effectifs scolaires</span>
    </div>

    {{-- Header --}}
    <div class="zn-header">
        <div class="zn-header-left">
            <h1>Effectifs scolaires</h1>
            <p>Gestion des effectifs par école, niveau et année</p>
        </div>
        @if(in_array(auth()->user()->role, ['admin', 'delegue', 'rbo']))
            <div class="zn-header-actions">
                <a href="{{ route('effectifs.create') }}" class="btn-zn btn-zn-primary">
    ...
    <div class="zn-search-bar">
        <form method="GET" action="{{ route('effectifs.index') }}" style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: flex-end;">
            <div class="zn-filter-group">
                <label class="zn-filter-label">Établissement</label>
            ...
            <div class="zn-filter-group">
                <label class="zn-filter-label">Niveau</label>
                <div class="frm-select-wrap">
                    <select name="niveau" class="zn-select">
            ...
            <div class="filter-actions">
                ...
                <a href="{{ route('effectifs.index') }}" class="btn-zn btn-zn-sm btn-zn-danger-ghost">
    ...
    <div class="zn-card">
        <div class="zn-card-header">
            ...
            <span class="zn-count">{{ $effectifs->total() }} ligne(s)</span>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($e->source_1)
                                <div class="source-line">
                                    {{ optional($e->sourceContact1)->prenom }} {{ optional($e->sourceContact1)->nom }} : {{ $e->nombre_classes_1 }} classe(s)
                                </div>
                            @endif
                            @if($e->source_2)
                                <div class="source-line">
                                    {{ optional($e->sourceContact2)->prenom }} {{ optional($e->sourceContact2)->nom }} : {{ $e->nombre_classes_2 }} classe(s)
                                </div>
                            @endif
                            @if($e->source_3)
                                <div class="source-line">
                                    {{ optional($e->sourceContact3)->prenom }} {{ optional($e->sourceContact3)->nom }} : {{ $e->nombre_classes_3 }} classe(s)
                                </div>
                            @endif
                            @if(!$e->source_1 && !$e->source_2 && !$e->source_3)
                                <span class="text-muted">—</span>
                            @endif
                        </td>
                        <td>
                            @if($e->effectif_valide)
                                <span class="dr-badge bd-teal" style="font-weight:600;">{{ $e->effectif_valide }}</span>
                            @else
                                <span class="text-muted">—</span>
                            @endif
                        </td>
                        ...
                        <td>
                            <div class="actions-cell">
                                @if(!$e->is_validated && (auth()->user()->role === 'delegue' && $e->compte->delegue_id === auth()->id() || in_array(auth()->user()->role, ['admin','rbo'])))
                                    <a href="{{ route('effectifs.edit', $e) }}" class="btn-zn btn-zn-sm btn-zn-warning">
                                ...
                                @if(!$e->is_validated && in_array(auth()->user()->role, ['admin','rbo']))
                                    <form action="{{ route('effectifs.validate', $e) }}" method="POST" style="display:inline;">
                                        @csrf
                                        <button type="submit" class="btn-zn btn-zn-sm btn-zn-primary">
                                            <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                @endif
                                @if($e->is_validated && auth()->user()->role === 'admin')
                                    <form action="{{ route('effectifs.devalidate', $e) }}" method="POST" style="display:inline;">
                                        @csrf
                                ...
                                @if(auth()->user()->role === 'admin')
                                    <form action="{{ route('effectifs.destroy', $e) }}" method="POST" onsubmit="return confirm('Supprimer définitivement cette ligne ?')" style="display:inline;">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn-zn btn-zn-sm btn-zn-danger">
                                            <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="zn-empty">
                                    ...
                                    <h3>Aucun effectif trouvé</h3>
                                    <p>{{ request('compte_id') || request('annee_scolaire_id') || request('niveau') ? 'Aucun résultat pour les filtres sélectionnés.' : 'Commencez par créer une ligne d\'effectif.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if($effectifs->hasPages())
            <div class="zn-pagination">
                {{ $effectifs->withQueryString()->links('vendor.pagination.custom') }}
            </div>
        @endif
    </div>
</div>
