...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <a href="#">Stocks</a>
        <span class="zn-bc-sep">›</span>
        <span class="zn-bc-cur">Consignation</span>
    </div>

    {{-- Header --}}
    <div class="zn-header">
        <div class="zn-header-left">
            <h1>Consignation (stocks délégués)</h1>
            <p>Gérez les stocks de spécimens par délégué, produit et année scolaire</p>
        </div>
        @if(auth()->user()->role === 'admin')
        <div class="zn-header-actions">
            <a href="{{ route('consignations.create') }}" class="btn-zn btn-zn-primary">
    ...
    <div class="zn-search-bar">
        <form method="GET" action="{{ route('consignations.index') }}" style="display:flex; gap:1rem; flex-wrap:wrap; align-items:flex-end;">
            ...
            @endif
            <div class="zn-filter-group">
                <label class="zn-filter-label">Produit</label>
                <select name="product_id" class="zn-select">
                    <option value="">Tous</option>
                    @foreach($products as $prod)
                        <option value="{{ $prod->id }}" {{ request('product_id') == $prod->id ? 'selected' : '' }}>{{ $prod->titre }}</option>
                    @endforeach
                </select>
            </div>
            <div class="zn-filter-group">
                ...
                <select name="annee_scolaire_id" class="zn-select">
                    @foreach($years as $year)
                        <option value="{{ $year->id }}" {{ ($yearId ?? '') == $year->id ? 'selected' : '' }}>{{ $year->libelle }}</option>
                    @endforeach
                </select>
            </div>
            <div class="filter-actions">
                ...
                <a href="{{ route('consignations.index') }}" class="btn-zn btn-zn-danger-ghost">
    ...
    <div class="zn-card">
        <div class="zn-card-header">
            ...
            <span class="zn-count">{{ $consignations->total() }} ligne(s)</span>
        </div>
        ...
        <div style="overflow-x: auto;">
            <table class="zn-table">
                ...
                <tbody>
                    @forelse($consignations as $cons)
                    <tr>
                        <td>{{ $cons->delegate->prenom }} {{ $cons->delegate->nom }}</td>
                        <td>
                            <strong>{{ $cons->product->titre }}</strong><br>
                            <small style="color:var(--text-muted); font-size:0.7rem;">{{ $cons->product->isbn_13 ?? $cons->product->isbn_10 }}</small>
                        </td>
                        <td>{{ $cons->anneeScolaire->libelle }}</td>
                        <td><strong style="color:var(--blue); font-size:1.1rem;">{{ $cons->quantity }}</strong></td>
                        <td>
                            <div class="actions-cell">
                                <a href="{{ route('consignations.create-bss', $cons) }}" class="btn-zn btn-zn-sm btn-zn-primary" title="Créer un BSS">
                                ...
                                @if(auth()->user()->role === 'admin')
                                    <a href="{{ route('consignations.edit', $cons) }}" class="btn-zn btn-zn-sm btn-zn-warning" title="Modifier">
                                        <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
                                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/>
                                        </svg>
                                    </a>
                                    <form action="{{ route('consignations.destroy', $cons) }}" method="POST" style="display:inline;" onsubmit="return confirm('Supprimer cette ligne ?')">
                                        @csrf @method('DELETE')
                        ...
                        <tr>
                            <td colspan="5">
                                <div class="zn-empty">
                                    ...
                                    <h3>Aucune ligne de consignation</h3>
        ...
        @if($consignations->hasPages())
            <div class="zn-pagination">
                {{ $consignations->withQueryString()->links('vendor.pagination.custom') }}
            </div>
        @endif
    </div>
</div>
