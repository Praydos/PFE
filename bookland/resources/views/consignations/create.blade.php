...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <span class="zn-bc-sep">›</span>
        <span class="zn-bc-cur">{{ isset($consignation) ? 'Modifier le stock' : 'Ajouter un stock' }}</span>
    </div>

    <div class="zn-header">
        <h1>{{ isset($consignation) ? 'Modifier le stock' : 'Ajouter un stock' }}</h1>
        <p>{{ isset($consignation) ? 'Mettez à jour les informations du stock.' : 'Remplissez les informations ci‑dessous pour créer un nouveau stock (consignation).' }}</p>
    </div>
    ...
    <div class="fp-card">
        <form method="POST" action="{{ isset($consignation) ? route('consignations.update', $consignation) : route('consignations.store') }}">
            @csrf
            @if(isset($consignation)) @method('PUT') @endif
            ...
            <div class="fp-section">
                <div class="fp-section-head">
                    ...
                    <div class="fp-section-meta">
                        <div class="fp-section-title">Détails du stock</div>
                        <div class="fp-section-sub">Délégué, produit, année scolaire et quantité</div>
                    </div>
                </div>
                ...
                <div class="fp-row fp-row-3">
                    {{-- Délégué --}}
                    <div class="frm-group">
                        <label class="frm-label" for="delegate_id">Délégué <span class="req">*</span></label>
                        <div class="frm-select-wrap">
                            <select name="delegate_id" id="delegate_id" class="frm-select {{ $errors->has('delegate_id') ? 'is-invalid' : '' }}" required>
                        ...
                        @error('delegate_id')<span class="frm-error">{{ $message }}</span>@enderror
                    </div>
                            ...
                            <select name="product_id" id="product_id" class="frm-select {{ $errors->has('product_id') ? 'is-invalid' : '' }}" required>
                                ...
                                @foreach($products as $prod)
                                    <option value="{{ $prod->id }}" {{ old('product_id', $consignation->product_id ?? '') == $prod->id ? 'selected' : '' }}>
                                        {{ $prod->titre }} ({{ $prod->isbn_13 ?? $prod->isbn_10 ?? 'N/R' }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="frm-group">
                        ...
                        <div class="frm-input-wrap">
                            ...
                            <input type="number" name="quantity" id="quantity"
                                   class="frm-input {{ $errors->has('quantity') ? 'is-invalid' : '' }}"
                                   value="{{ old('quantity', $consignation->quantity ?? 0) }}" min="0" step="1" required>
                        </div>
            ...
            <div class="fp-footer">
                ...
                <a href="{{ route('consignations.index') }}" class="btn-zn btn-zn-ghost">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <button type="submit" class="btn-zn btn-zn-primary">
                    ...
                    {{ isset($consignation) ? 'Enregistrer les modifications' : 'Enregistrer' }}
                </button>
            </div>
        </form>
    </div>
