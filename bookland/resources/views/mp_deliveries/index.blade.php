...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Extra style for date inputs (not in original CSS) */
    .zn-input {
        width: 100%;
        padding: 0.55rem 0.85rem;
        border: 1px solid var(--border);
...
<div class="zn-page">
    ...
    <div class="zn-header">
        <div class="zn-header-left">
            <h1>Livraisons MP</h1>
            <p>Suivi des livraisons de matériel pédagogique par établissement.</p>
        </div>
        <div class="zn-header-actions">
            @if(auth()->user()->role === 'admin')
                <a href="{{ route('mp-products.index') }}" class="btn-zn btn-zn-ghost">
            ...
            @if(in_array(auth()->user()->role, ['admin', 'rbo', 'delegue']))
                <a href="{{ route('mp-deliveries.create') }}" class="btn-zn btn-zn-primary">
    ...
    <div class="zn-search-bar">
        <form method="GET" action="{{ route('mp-deliveries.index') }}" style="display:flex; gap:1rem; flex-wrap:wrap; align-items:flex-end;">
            ...
            <div class="zn-filter-group">
                <label class="zn-filter-label">Du</label>
                <input type="date" name="date_from" value="{{ request('date_from') }}" class="zn-input">
            </div>
            <div class="zn-filter-group">
                <label class="zn-filter-label">Au</label>
                <input type="date" name="date_to" value="{{ request('date_to') }}" class="zn-input">
            </div>
            ...
            <div class="filter-actions">
                ...
                <a href="{{ route('mp-deliveries.index') }}" class="btn-zn btn-zn-danger-ghost">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
    ...
    <div class="zn-card">
        <div class="zn-card-header">
            ...
            <span class="zn-count">{{ $deliveries->total() }} livraison(s)</span>
        </div>
                ...
                <tbody>
                    @forelse($deliveries as $row)
                        <tr>
                            ...
                            <td>{{ $row->compte?->etablissement ?? '—' }}</td>
                            <td>{{ $row->contact ? trim($row->contact->prenom.' '.$row->contact->nom) : '—' }}</td>
                            <td>
                                {{ $row->mpProduct?->nom ?? '—' }}
                                @if($row->mpProduct?->code_article)
                                    <span style="color:var(--text-muted); font-size:.7rem;">({{ $row->mpProduct->code_article }})</span>
                                @endif
                            </td>
                            <td>{{ $row->date_delivery?->format('d/m/Y') }}</td>
                            <td>
                                @if($row->statut === 'livre')
                            ...
                            <td class="actions-cell">
                                <a href="{{ route('mp-deliveries.show', $row) }}" class="btn-zn btn-zn-sm btn-zn-info">
                                ...
                                @if(auth()->user()->role === 'admin')
                                    <form action="{{ route('mp-deliveries.destroy', $row) }}" method="POST" onsubmit="return confirm('Supprimer cette livraison MP ?')" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="zn-empty">
                                    ...
                                    <h3>Aucune livraison MP</h3>
                                    <p>{{ request('compte_id') || request('delegate_id') || request('date_from') || request('date_to') ? 'Aucune livraison pour les filtres sélectionnés.' : 'Commencez par créer une nouvelle livraison.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
@push('scripts')
{{-- No extra scripts needed for this index --}}
@endpush