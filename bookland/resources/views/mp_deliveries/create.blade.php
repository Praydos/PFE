...
<div class="zn-page">
    ...
    <div class="zn-header">
        ...
        <p>Année scolaire active : <strong>{{ $currentYear->libelle ?? '—' }}</strong></p>
    </div>
    ...
    <div class="fp-card">
        <form method="POST" action="{{ route('mp-deliveries.store') }}" id="mp-delivery-form">
            @csrf
            ...
            <div class="fp-section">
                <div class="fp-section-head">
                    ...
                    <div class="fp-section-meta">
                        <div class="fp-section-title">Informations de livraison</div>
                        <div class="fp-section-sub">Compte, contact, matériel pédagogique et date</div>
                    </div>
                </div>
                ...
                <div class="fp-row fp-row-2">
                    {{-- Compte --}}
                    <div class="frm-group">
                        <label class="frm-label" for="compte_id">Compte (établissement) <span class="req">*</span></label>
                        <div class="frm-select-wrap">
                            <select name="compte_id" id="compte_id" class="frm-select @error('compte_id') is-invalid @enderror" required>
                                ...
                                @foreach($comptes as $c)
                                    <option value="{{ $c->id }}" {{ old('compte_id') == $c->id ? 'selected' : '' }}>
                                        {{ $c->etablissement }} @if($c->ville) ({{ $c->ville->nom }}) @endif
                                    </option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="fp-row fp-row-2">
                    ...
                    <div class="frm-group">
                        <label class="frm-label" for="mp_product_id">Matériel pédagogique <span class="req">*</span></label>
                        <div class="frm-select-wrap">
                            <select name="mp_product_id" id="mp_product_id" class="frm-select @error('mp_product_id') is-invalid @enderror" required>
                                <option value="">— Choisir —</option>
                                @foreach($products as $p)
                                    <option value="{{ $p->id }}" {{ old('mp_product_id') == $p->id ? 'selected' : '' }}>
                                        {{ $p->nom }} — {{ $p->code_article }} ({{ $p->editeur }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        @error('mp_product_id')<span class="frm-error">{{ $message }}</span>@enderror
                    </div>

                    {{-- Date de livraison --}}
                    <div class="frm-group">
                        <label class="frm-label" for="date_delivery">Date de livraison <span class="req">*</span></label>
                        <input type="date" name="date_delivery" id="date_delivery"
                               class="frm-input @error('date_delivery') is-invalid @enderror"
                               value="{{ old('date_delivery', $defaultDate ?? now()->format('Y-m-d')) }}" required>
                        @error('date_delivery')<span class="frm-error">{{ $message }}</span>@enderror
                    </div>
                </div>
            </div>
