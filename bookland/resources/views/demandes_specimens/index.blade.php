...
@push('styles')
    ...
    <style>
        ...
        *::after {
        ...
        .zn-pagination .page-link:hover {
            background: var(--bg-hover);
            border-color: var(--border-md);
        }

        .zn-pagination .active .page-link {
            background: var(--blue);
            border-color: var(--blue);
            color: white;
        }

        .zn-pagination .disabled .page-link {
            opacity: 0.5;
            pointer-events: none;
        }
...
@section('content')
    <div class="zn-page">
        ...
        <div class="zn-search-bar">
            <form method="GET" action="{{ route('demandes-specimens.index') }}"
                    ...
                    <div class="frm-select-wrap">
                        <select name="statut" class="zn-select">
                            ...
                            @foreach($statuts as $s)
                                <option value="{{ $s }}" {{ request('statut') == $s ? 'selected' : '' }}>{{ ucfirst($s) }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <table class="zn-table">
                    ...
                    <tbody>
                        @forelse($demandes as $d)
                            <tr style="{{ $d->statut !== 'demande' ? 'background-color: rgba(0,0,0,.01);' : '' }}">
                                <td>
                                    <span class="dr-badge bd-blue">{{ ucfirst($d->type) }}</span>
                                </td>
                                <td>
                                    @if($d->type == 'etablissement')
                                        <strong>{{ $d->compte->etablissement ?? '-' }}</strong>
                                    @else
                                        ...
                                        @if($d->statut === 'valide')
                                            <span class="locked-indicator">
                                                <svg width="10" height="10" fill="currentColor" viewBox="0 0 24 24">
                                                    <path
                                                        d="M12 1c-6.338 0-12 4.226-12 10.007 0 2.05.738 4.063 2.047 5.625.055 3.215 2.902 5.368 9.953 5.368 1.668 0 4.281-.105 5.953-.747.827 1.536 2.456 2.754 4.047 2.754.828 0 1.625-.371 2.25-1.057l1.75-1.944.75 1.944c.55.828 1.522 1.057 2.25 1.057 1.591 0 3.22-1.218 4.047-2.754 1.672.642 4.285.747 5.953.747 7.051 0 9.898-2.153 9.953-5.368 1.309-1.562 2.047-3.575 2.047-5.625 0-5.781-5.662-10.007-12-10.007zm.623 17.769c-3.905.504-8.721.504-12.623 0-.964-1.116-1.5-2.401-1.5-3.769 0-3.314 3.134-6 7-6s7 2.686 7 6c0 1.368-.536 2.653-1.5 3.769z"
                                                        transform="scale(.8)" />
                                                </svg>
                                                Verrouillée
                                            </span>
                                ...
                                <td>
                                    @if($d->originalBss)
                                        <a href="{{ route('bss.show', $d->originalBss) }}" class="btn-zn btn-zn-sm btn-zn-info"
                                            title="Voir le BSS original">
                                            {{ $d->originalBss->numero }}
                                        </a>
                                    @else
                                ...
                                <td>
                                    <div class="actions-cell">
                                        <a href="{{ route('demandes-specimens.show', $d) }}"
                                            class="btn-zn btn-zn-sm btn-zn-info">
                                        ...
                                        @if($d->statut === 'demande')
                                            @if(in_array(auth()->user()->role, ['admin', 'rbo']))
                                                <form method="POST" action="{{ route('demandes-specimens.validate', $d) }}"
                                                    style="display:inline;">
                                                    @csrf
                                                    <button type="submit" name="action" value="approve"
                                                        class="btn-zn btn-zn-sm btn-zn-success"
                                                        onclick="return confirm('Approuver cette demande et créer un BSS ?')">
                                                        <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"
                                                            viewBox="0 0 24 24">
                                                ...
                                                <form method="POST" action="{{ route('demandes-specimens.validate', $d) }}"
                                                    ...
                                                    <button type="submit" name="action" value="decline"
                                                        class="btn-zn btn-zn-sm btn-zn-danger"
                                            ...
                                            @if((auth()->user()->role === 'delegue' && $d->delegue_id === auth()->id()) || auth()->user()->role === 'admin')
                                                ...
                                                <form method="POST" action="{{ route('demandes-specimens.destroy', $d) }}"
                                                    style="display:inline;">
                                                    @csrf @method('DELETE')
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="zn-empty">
                                        ...
                                        <p>{{ request('statut') || request('type') || request('compte_id') || request('delegue_id') ? 'Aucun résultat pour les filtres sélectionnés.' : 'Commencez par créer une nouvelle demande.' }}
                                        </p>
                                    </div>
                                </td>
                            </tr>
