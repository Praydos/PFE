...
<style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-zn-disabled { opacity: 0.5; pointer-events: none; cursor: not-allowed; }
    ...
    .zn-filter-group { display: flex; flex-direction: column; gap: 0.35rem; min-width: 160px; }
    .zn-filter-label { font-size: 0.7rem; font-weight: 600; color: var(--text-secondary); letter-spacing: 0.03em; text-transform: uppercase; }
    .zn-select {
        width: 100%; padding: 0.55rem 0.85rem; border: 1px solid var(--border);
    ...
    .zn-select:focus { border-color: var(--blue); box-shadow: 0 0 0 3px var(--blue-mid); }
    .filter-actions { display: flex; align-items: flex-end; gap: 0.6rem; flex-wrap: wrap; }
    ...
    .frm-select-wrap .zn-select { padding-right: 2.2rem; }
    ...
    .locked-indicator {
        ...
        background: rgba(232,80,106,.08); color: var(--rose);
        font-size: .7rem; font-weight: 600;
    }

    .zn-empty { padding: 4rem 2rem; text-align: center; }
    .zn-empty-icon { width: 52px; height: 52px; border-radius: var(--r-md); background: var(--bg-subtle); border: 1px solid var(--border); display: flex; align-items: center; justify-content: center; margin: 0 auto 1rem; }
    ...
    .zn-pagination .page-link {
        ...
        color: var(--text-secondary); font-size: 0.8rem; text-decoration: none; transition: all var(--t);
    }
            ...
            <table class="zn-table">
                ...
                <tbody>
                    ...
                    <tr style="{{ $d->statut !== 'demande' ? 'background-color: rgba(0,0,0,.01);' : '' }}">
                        ...
                        <td>
                            @if($d->originalBss)
                                <a href="{{ route('bss.show', $d->originalBss) }}" class="btn-zn btn-zn-sm btn-zn-info" title="Voir le BSS original">
                                ...
                                @if($d->statut === 'demande')
                                    @if(in_array(auth()->user()->role, ['admin','rbo']))
                                        <form method="POST" action="{{ route('demandes-specimens.validate', $d) }}" style="display:inline;">
                                            @csrf
                                            <button type="submit" name="action" value="approve" class="btn-zn btn-zn-sm btn-zn-success" onclick="return confirm('Approuver cette demande et créer un BSS ?')">
                                                <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    ...
                                    @if(auth()->user()->role === 'delegue' && $d->delegue_id === auth()->id())
                                        <a href="{{ route('demandes-specimens.edit', $d) }}" class="btn-zn btn-zn-sm btn-zn-ghost" style="border-color: var(--amber); color: var(--amber);">
                                        ...
                                        <form method="POST" action="{{ route('demandes-specimens.destroy', $d) }}" style="display:inline;">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="btn-zn btn-zn-sm btn-zn-danger" onclick="return confirm('Supprimer définitivement ?')" title="Supprimer">
                                                <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
