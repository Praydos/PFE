...
@push('styles')
{{-- Same <link> and <style> as above --}}
<link href="https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,400;0,9..40,500;0,9..40,600;0,9..40,700;1,9..40,400&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet">
<style>
    /* Full CSS from create view – paste it here */
...
<div class="form-page">
    ...
    <div class="fp-header">
        ...
        <p>Mettez à jour les informations de l'établissement <strong>{{ $compte->etablissement }}</strong>.</p>
    </div>
    ...
    <div class="fp-card">
        <div class="fp-card-header">
            ...
            <span class="fp-card-title">Informations du compte</span>
        </div>

        <form method="POST" action="{{ route('comptes.update', $compte) }}" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="fp-footer">
                <a href="{{ route('comptes.index') }}" class="btn-fp btn-fp-ghost">
...
(function () {
    ...
    function filterDelegates() {
        ...
        // Restore selected delegate if still valid
        const currentSelected = "{{ old('delegue_id', $compte->delegue_id ?? '') }}";
        if (currentSelected && [...delegateSelect.options].some(opt => opt.value === currentSelected)) {
            delegateSelect.value = currentSelected;
        }
    }
