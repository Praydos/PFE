    ...
    <div class="zn-card">
        ...
        <div class="card-footer" style="padding: 1.1rem 1.6rem; border-top: 1px solid var(--border); background: var(--bg-base); display: flex; justify-content: flex-end; gap: 0.6rem;">
            ...
            @if(auth()->user()->role === 'admin')
                <form method="POST" action="{{ route('examens.destroy', $examen) }}" style="display:inline;" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cet examen ?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-zn btn-zn-danger" title="Supprimer">
                        <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                        Supprimer
                    </button>
                </form>
            @endif
