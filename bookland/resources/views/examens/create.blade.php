...
<style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .frm-input::placeholder, .frm-textarea::placeholder { color: var(--text-hint); }
    .frm-input:focus, .frm-select:focus, .frm-textarea:focus {
        border-color: var(--blue);
        box-shadow: 0 0 0 3px var(--blue-mid);
    }
    .frm-input:hover:not(:focus), .frm-select:hover:not(:focus), .frm-textarea:hover:not(:focus) {
        border-color: var(--border-md);
    }
    .frm-textarea { resize: vertical; min-height: 76px; }
    .frm-hint { font-size: .72rem; color: var(--text-muted); margin-top: .2rem; }
...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <span class="zn-bc-sep">›</span>
        <span class="zn-bc-cur">{{ isset($examen) ? 'Modifier la demande' : 'Nouvelle demande' }}</span>
    </div>

    <div class="zn-header">
        <h1>{{ isset($examen) ? 'Modifier la demande' : 'Nouvelle demande d\'examen' }}</h1>
        <p>{{ isset($examen) ? 'Mettez à jour les informations de la demande.' : 'Remplissez les informations ci-dessous pour créer une demande.' }}</p>
    </div>
        ...
        <form method="POST" action="{{ isset($examen) ? route('examens.update', $examen) : route('examens.store') }}" id="exam-form">
            ...
            <div class="fp-section" id="sec-epreuves">
                ...
                <div class="ep-table-wrap">
                    ...
                    <div id="epreuves-container">
                        @if(isset($examen) && $examen->epreuves->count())
            ...
            <div class="fp-footer">
                ...
                <button type="submit" class="btn-zn btn-zn-primary">
                    ...
                    {{ isset($examen) ? 'Enregistrer les modifications' : 'Créer la demande' }}
                </button>
            </div>
...
(function () {
    ...
    // ── Épreuves dynamic rows ─────────────────────
    let epIdx = {{ isset($examen) ? $examen->epreuves->count() : 1 }};
    const container = document.getElementById('epreuves-container');
    const countBadge = document.getElementById('nav-ep-count'); // may be null, safe

    function updateEpCount() {
        const n = container.querySelectorAll('.epreuve-row').length;
