...
<div class="zn-page">
    ...
    <div class="zn-header">
        ...
        <p>Action n° {{ $actions_amelioration->numero }} – Mettez à jour les informations ci-dessous.</p>
    </div>
        ...
        <form method="POST" action="{{ route('actions-amelioration.update', $actions_amelioration) }}">
            ...
            <div class="fp-section">
                        ...
                        <div class="frm-select-wrap">
                            <select name="compte_id" id="compte_id" class="frm-select @error('compte_id') is-invalid @enderror" required>
                                ...
                                @foreach($comptes as $c)
                                    <option value="{{ $c->id }}" {{ old('compte_id', $actions_amelioration->compte_id) == $c->id ? 'selected' : '' }}>
                                        {{ $c->etablissement }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="fp-row fp-row-3">
                    <div class="frm-group">
                        ...
                        <input type="date" name="dateAA" id="dateAA" class="frm-input @error('dateAA') is-invalid @enderror"
                               value="{{ old('dateAA', $actions_amelioration->dateAA->format('Y-m-d')) }}" required>
                    ...
                    <div class="frm-group">
                        ...
                        <div class="frm-select-wrap">
                            <select name="type" id="type" class="frm-select @error('type') is-invalid @enderror" required>
                                <option value="">-- Sélectionnez --</option>
                                @foreach($types as $t)
                                    <option value="{{ $t }}" {{ old('type', $actions_amelioration->type) == $t ? 'selected' : '' }}>{{ $t }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="frm-select-wrap">
                            <select name="origine" id="origine" class="frm-select @error('origine') is-invalid @enderror" required>
                                ...
                                @foreach($origines as $o)
                                    <option value="{{ $o }}" {{ old('origine', $actions_amelioration->origine) == $o ? 'selected' : '' }}>{{ $o }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="frm-group">
                    ...
                    <textarea name="analyse_causes" id="analyse_causes" class="frm-textarea @error('analyse_causes') is-invalid @enderror" rows="2">{{ old('analyse_causes', $actions_amelioration->analyse_causes) }}</textarea>
                ...
                <div class="frm-group">
                    ...
                    <textarea name="sanctions" id="sanctions" class="frm-textarea @error('sanctions') is-invalid @enderror" rows="2">{{ old('sanctions', $actions_amelioration->sanctions) }}</textarea>
                ...
                <div class="frm-group">
                    ...
                    <textarea name="resultats_attendus" id="resultats_attendus" class="frm-textarea @error('resultats_attendus') is-invalid @enderror" rows="2">{{ old('resultats_attendus', $actions_amelioration->resultats_attendus) }}</textarea>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const currentEmetteurId = '{{ old('emetteur_id', $actions_amelioration->emetteur_id) }}';
        ...
        function loadContacts() {
            ...
            fetch(`/api/comptes/${compteId}/contacts`)
                ...
                .then(data => {
                    ...
                    if (currentEmetteurId) {
                        emetteurSelect.value = currentEmetteurId;
                    }
                })
