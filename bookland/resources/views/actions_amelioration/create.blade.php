...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <span class="zn-bc-sep">›</span>
        @if(isset($actions_amelioration))
            <a href="{{ route('actions-amelioration.show', $actions_amelioration) }}">{{ $actions_amelioration->id }}</a>
            <span class="zn-bc-sep">›</span>
    ...
    <div class="zn-header">
        <h1>{{ isset($actions_amelioration) ? 'Modifier l\'action d\'amélioration' : 'Nouvelle action d\'amélioration' }}</h1>
        <p>{{ isset($actions_amelioration) ? 'Mettez à jour les informations de l\'action.' : 'Remplissez les informations ci‑dessous pour créer une action d\'amélioration.' }}</p>
    </div>
    ...
    @php
        ...
        $action = $actions_amelioration ?? null;
    @endphp

    <div class="fp-card">
        <form method="POST" action="{{ $isEdit ? route('actions-amelioration.update', $action) : route('actions-amelioration.store') }}">
            @csrf
            ...
            <div class="fp-section">
                <div class="fp-section-head">
                    ...
                    <div class="fp-section-meta">
                        ...
                        <div class="fp-section-sub">Compte, émetteur, date, type et origine</div>
                    </div>
                </div>

                {{-- Compte and Émetteur (2 columns) --}}
                <div class="fp-row fp-row-2">
                        ...
                        <div class="frm-select-wrap">
                            <select name="compte_id" id="compte_id" class="frm-select @error('compte_id') is-invalid @enderror" required>
                                ...
                                @foreach($comptes as $c)
                                    <option value="{{ $c->id }}" {{ old('compte_id', $isEdit ? $action->compte_id : '') == $c->id ? 'selected' : '' }}>
                                        {{ $c->etablissement }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="frm-group">
                        <label class="frm-label" for="emetteur_id">Émetteur (contact) <span class="req">*</span></label>
                        <div class="frm-select-wrap">
                            <select name="emetteur_id" id="emetteur_id" class="frm-select @error('emetteur_id') is-invalid @enderror" required>
                                <option value="">-- Sélectionnez d'abord un compte --</option>
                            </select>
                        </div>
                        @error('emetteur_id')<span class="frm-error">{{ $message }}</span>@enderror
                    </div>
                </div>

                {{-- Date AA, Type, Origine (3 columns) --}}
                <div class="fp-row fp-row-3">
                    <div class="frm-group">
                        <label class="frm-label" for="dateAA">Date AA <span class="req">*</span></label>
                        <input type="date" name="dateAA" id="dateAA" class="frm-input @error('dateAA') is-invalid @enderror"
                               value="{{ old('dateAA', $isEdit ? $action->dateAA->format('Y-m-d') : '') }}" required>
                        @error('dateAA')<span class="frm-error">{{ $message }}</span>@enderror
                    </div>

                    <div class="frm-group">
                        <label class="frm-label" for="type">Type <span class="req">*</span></label>
                        <div class="frm-select-wrap">
                            <select name="type" id="type" class="frm-select @error('type') is-invalid @enderror" required>
                                <option value="">-- Sélectionnez --</option>
                                @foreach($types as $t)
                                    <option value="{{ $t }}" {{ old('type', $isEdit ? $action->type : '') == $t ? 'selected' : '' }}>{{ $t }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="frm-group">
                        <label class="frm-label" for="origine">Origine <span class="req">*</span></label>
                        <div class="frm-select-wrap">
                            <select name="origine" id="origine" class="frm-select @error('origine') is-invalid @enderror" required>
                                <option value="">-- Sélectionnez --</option>
                                @foreach($origines as $o)
                                    <option value="{{ $o }}" {{ old('origine', $isEdit ? $action->origine : '') == $o ? 'selected' : '' }}>{{ $o }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('origine')<span class="frm-error">{{ $message }}</span>@enderror
                    </div>
                </div>

                {{-- Analyse des causes --}}
                <div class="frm-group">
                    <label class="frm-label" for="analyse_causes">Analyse des causes</label>
                    <textarea name="analyse_causes" id="analyse_causes" class="frm-textarea @error('analyse_causes') is-invalid @enderror" rows="2">{{ old('analyse_causes', $isEdit ? $action->analyse_causes : '') }}</textarea>
                    @error('analyse_causes')<span class="frm-error">{{ $message }}</span>@enderror
                </div>

                {{-- Sanctions --}}
                <div class="frm-group">
                    <label class="frm-label" for="sanctions">Sanctions</label>
                    <textarea name="sanctions" id="sanctions" class="frm-textarea @error('sanctions') is-invalid @enderror" rows="2">{{ old('sanctions', $isEdit ? $action->sanctions : '') }}</textarea>
                    @error('sanctions')<span class="frm-error">{{ $message }}</span>@enderror
                </div>

                {{-- Résultats attendus --}}
                <div class="frm-group">
                    <label class="frm-label" for="resultats_attendus">Résultats attendus</label>
                    <textarea name="resultats_attendus" id="resultats_attendus" class="frm-textarea @error('resultats_attendus') is-invalid @enderror" rows="2">{{ old('resultats_attendus', $isEdit ? $action->resultats_attendus : '') }}</textarea>
                    @error('resultats_attendus')<span class="frm-error">{{ $message }}</span>@enderror
                </div>
            </div>
            ...
            <div class="fp-footer">
                ...
                <a href="{{ route('actions-amelioration.index') }}" class="btn-zn btn-zn-ghost">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function loadContacts() {
            ...
            fetch(`/api/comptes/${compteId}/contacts`)
                ...
                .then(data => {
                    ...
                    // Restore previously selected contact if any (for edit mode)
                    const defaultEmetteurId = '{{ old('emetteur_id', $isEdit ? $action->emetteur_id : '') }}';
                    if (defaultEmetteurId) {
                        emetteurSelect.value = defaultEmetteurId;
                    }
                })
        ...
        // Load contacts on page load if a compte is already selected (edit mode)
