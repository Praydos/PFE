...
<div class="dr-page">
    ...
    <form method="POST" action="{{ route('actions-amelioration.update-efficacite', $actions_amelioration) }}">
        @csrf @method('PUT')
        <div class="row">
            <div class="col-md-6 mb-3"><label>Date efficacité</label><input type="date" name="date_efficacite" class="form-control" value="{{ old('date_efficacite', $actions_amelioration->date_efficacite ? $actions_amelioration->date_efficacite->format('Y-m-d') : '') }}"></div>
            <div class="col-md-6 mb-3">
                <label>Responsable efficacité *</label>
                <select name="responsable_efficacite_id" class="form-select" required>
                    <option value="">-- Sélectionnez un contact --</option>
                    @foreach($contacts as $c)
                        <option value="{{ $c->id }}" {{ old('responsable_efficacite_id', $actions_amelioration->responsable_efficacite_id) == $c->id ? 'selected' : '' }}>
        ...
        <div class="row">
            <div class="col-md-4 mb-3"><label>Action efficace ?</label><select name="action_efficace" class="form-select"><option value="">-- Non spécifié --</option><option value="1" {{ old('action_efficace', $actions_amelioration->action_efficace) === '1' ? 'selected' : '' }}>Oui</option><option value="0" {{ old('action_efficace', $actions_amelioration->action_efficace) === '0' ? 'selected' : '' }}>Non</option></select></div>
