...
<div class="zn-page">
    ...
    <div class="zn-header">
        ...
        <p>Mettez à jour les informations de la zone <strong>{{ $zone->name }}</strong>.</p>
    </div>
    ...
    <div class="fp-card">
        <form method="POST" action="{{ route('zones.update', $zone) }}" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="fp-section">
                <div class="fp-section-head">
                    ...
                    <div class="fp-section-meta">
                        <div class="fp-section-title">Informations de la zone</div>
                        <div class="fp-section-sub">Nom, ville, RBO et délégués</div>
                    </div>
                </div>

                {{-- Include the existing _form partial (fields) --}}
                @include('zones._form')
            </div>
            ...
            <div class="fp-footer">
                ...
                <a href="{{ route('zones.index') }}" class="btn-zn btn-zn-ghost">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentRboId = {{ $zone->rbo_id ?? 'null' }};

    // Function to load RBOs for a given ville
    ...
    // Only reload when the city changes
    villeSelect.addEventListener('change', function() {
        const villeId = this.value;
        loadRbos(villeId, function() {
            if (currentRboId && rboSelect.querySelector(`option[value="${currentRboId}"]`)) {
                rboSelect.value = currentRboId;
            }
        });
    });

    // Do NOT load on page load – the server-side options already contain the correct selection.
    // This preserves the initial RBO selection in edit mode.
});
</script>
@endpush