...
/* ── Edit badge ─────────────────────────────────────── */
.ac-edit-badge { display:flex; align-items:center; gap:.6rem; padding:.75rem 1.1rem; background:var(--amber-l); border:1px solid rgba(232,160,32,.25); border-radius:var(--r3); margin-bottom:1.25rem; }
...
<div class="ac-page">
    ...
    <div class="ac-bc">
        ...
        <span class="ac-bc-s">›</span>
        <a href="{{ route('actions.show', $action) }}">{{ Str::limit($action->objet, 40) }}</a>
        <span class="ac-bc-s">›</span>
    ...
    <div class="ac-header">
        ...
        <p>Mise à jour de <strong>{{ $action->objet }}</strong></p>
    </div>
    ...
    <div class="ac-card">
        <div class="ac-card-hd">
            <span class="ac-card-pip amber"></span>
            <span class="ac-card-title">Formulaire de modification</span>
        </div>

        <form method="POST" action="{{ route('actions.update', $action) }}" id="actionForm" novalidate>
            @csrf
            @method('PUT')
            ...
            <div style="padding:0 1.6rem 1.6rem;">
                ...
                <div id="lines-container">
                    @foreach($action->lignes as $idx => $line)
                        <div class="line-item" data-line-index="{{ $idx }}">
                            @include('actions._line_form', [
                                'lineIndex' => $idx,
                                'line'      => $line,
                                'products'  => $products,
                                'examens'   => $examens,
                                'bssOptions'=> $bssOptions,
                            ])
                        </div>
                    @endforeach
            ...
            <div class="ac-footer">
                <a href="{{ route('actions.show', $action) }}" class="btn-ac btn-ac-ghost">
...
document.addEventListener('DOMContentLoaded', function () {
    let lineCounter   = {{ $action->lignes->count() ?: 1 }};
    ...
    /* ── API helpers ───────────────────────── */
    function loadContacts(compteId, selectEl, selectedIds) {
        ...
        fetch(`/api/comptes/${compteId}/contacts`)
            ...
            .then(data => {
                selectEl.innerHTML = data.map(c =>
                    `<option value="${c.id}" ${selectedIds.includes(c.id) ? 'selected' : ''}>
    ...
    function attachLineEvents(lineEl, isNew) {
        ...
        if (compteSelect?.value && contactSelect) {
            const selectedIds = isNew ? [] :
                Array.from(contactSelect.selectedOptions).map(o => parseInt(o.value));
            loadContacts(compteSelect.value, contactSelect, selectedIds);
        }
        ...
        if (categorieSelect && actionTypeSelect) {
            ...
            if (categorieSelect.value) {
                const savedActionType = actionTypeSelect.dataset.saved || actionTypeSelect.value || '';
    ...
    document.getElementById('add-line')?.addEventListener('click', () => {
        ...
        attachLineEvents(template, true);
    });
    ...
    document.querySelectorAll('.line-item').forEach(lineEl => attachLineEvents(lineEl, false));
});
</script>
@endpush