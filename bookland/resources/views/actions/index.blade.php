...
<style>
    ...
    /* Paste the complete CSS from the previous answer (the one with .zn-page, .zn-card, etc.) */
    /* For brevity, we assume it's included – you must copy the full block from the previous answer */
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-zn-primary:hover { background: var(--blue-dark); color: #fff; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(91,141,238,.4); text-decoration: none; }
    ...
    .zn-card-title { font-size: .88rem; font-weight: 700; color: var(--text-primary); display: flex; align-items: center; gap: .55rem; }
    ...
    .bd-planifie { background: var(--amber-light); color: var(--amber); }
    .bd-realise { background: var(--blue-light); color: var(--blue); }
    .bd-valide { background: var(--green-light); color: var(--green); }
    .bd-annule { background: var(--rose-light); color: var(--rose); }
    .bd-reporte { background: var(--violet-light); color: var(--violet); }
...
<div class="zn-page">
    ...
    <div class="zn-bc">
        ...
        <a href="#">Commercial</a>
        <span class="zn-bc-sep">›</span>
        <span class="zn-bc-cur">Actions</span>
    </div>
    ...
    <div class="zn-header">
        <div class="zn-header-left">
            ...
            <p>Gestion des actions et tâches</p>
        </div>
        @if(auth()->user()->role === 'delegue')
        <div class="zn-header-actions">
            <a href="{{ route('actions.create') }}" class="btn-zn btn-zn-primary">
    ...
    <div class="zn-search-bar">
        <form method="GET" action="{{ route('actions.index') }}" style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: flex-end;">
            ...
            <div class="filter-actions">
                ...
                <a href="{{ route('actions.index') }}" class="btn-zn btn-zn-sm btn-zn-danger-ghost">
    ...
    <div class="zn-card">
        <div class="zn-card-header">
            ...
            <span class="zn-count">{{ $actions->total() }} action(s)</span>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="actions-cell">
                                <a href="{{ route('actions.show', $a) }}" class="btn-zn btn-zn-sm btn-zn-ghost">
                                    <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                @if(in_array(auth()->user()->role, ['admin','rbo']) || (auth()->user()->role === 'delegue' && $a->delegue_id === auth()->id()))
                                    @if($a->statut === 'planifie')
                                        <a href="{{ route('actions.edit', $a) }}" class="btn-zn btn-zn-sm btn-zn-warning">
                                        ...
                                        <form method="POST" action="{{ route('actions.realiser', $a) }}" style="display:inline;">
                                            @csrf
                                            <button type="submit" class="btn-zn btn-zn-sm btn-zn-primary" onclick="return confirm('Marquer comme réalisée ?')">
                                                <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        ...
                                        <form method="POST" action="{{ route('actions.annuler', $a) }}" style="display:inline;">
                                            @csrf
                                            <button type="submit" class="btn-zn btn-zn-sm btn-zn-danger" onclick="return confirm('Annuler cette action ?')">
                                                <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                    <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
                                                </svg>
                                                Annuler
                                    ...
                                    @elseif($a->statut === 'realise' && in_array(auth()->user()->role, ['admin','rbo']))
                                        <form method="POST" action="{{ route('actions.valider', $a) }}" style="display:inline;">
                                            @csrf
                                            <button type="submit" class="btn-zn btn-zn-sm btn-zn-success" onclick="return confirm('Valider cette action ?')">
                                                <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                    <polyline points="20 6 9 17 4 12"/>
                                                </svg>
                                                Valider
        ...
        @if($actions->hasPages())
            <div class="zn-pagination">
                {{ $actions->withQueryString()->links('vendor.pagination.custom') }}
            </div>
        @endif
    </div>
</div>
