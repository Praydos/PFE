...
@push('styles')
    ...
    <style>
        ...
        *::after {
        ...
        .ai-select {
            width: 100%;
            padding: .52rem 2rem .52rem .85rem;
            border: 1px solid var(--border);
            border-radius: var(--r2);
            background: var(--card);
            font-family: var(--font);
...
@section('content')
    <div class="ai-page">
        ...
        {{-- Active filter pills --}}
        @if(request('statut') || request('type') || request('compte_id') || request('delegue_id'))
        ...
        <div class="ai-filters">
            <form method="GET" action="{{ route('actions.index') }}" style="display:contents;">
                ...
                @if(in_array(auth()->user()->role, ['admin', 'abo', 'rbo']))
                    <div class="ai-filter-grp">
                        <label class="ai-filter-label">Délégué</label>
                        <div class="ai-sel-wrap">
                            <select name="delegue_id" class="ai-select">
        ...
        <div class="ai-card">
            <div class="ai-card-hd">
                ...
                <span class="ai-count">
                    ...
                    @if(request('statut') || request('type') || request('compte_id') || request('delegue_id')) · filtrées
                    @endif
                </span>
            </div>
                                    ...
                                    <div class="actions-cell">
                                        ...
                                        @if(in_array(auth()->user()->role, ['admin', 'rbo']) || (auth()->user()->role === 'delegue' && $a->delegue_id === auth()->id()))
                                            ...
                                            @if($a->statut === 'planifie' && auth()->user()->role === 'delegue' && $a->delegue_id === auth()->id())
                                                <a href="{{ route('actions.show', $a) }}?realiser=1"
                                                    class="btn-ai btn-ai-sm btn-ai-success">
                                                    <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2"
                                                        viewBox="0 0 24 24">
                                            ...
                                            @if($a->statut === 'planifie' && (auth()->user()->role === 'admin' || (auth()->user()->role === 'delegue' && $a->delegue_id === auth()->id())))
                                                ...
                                                <form method="POST" action="{{ route('actions.annuler', $a) }}" style="display:inline;"
                                                    onsubmit="return confirm('Annuler cette action ?')">
                                                    @csrf
                                            ...
                                            @if($a->statut === 'realise' && in_array(auth()->user()->role, ['admin', 'rbo']))
                                                ...
                                                @if($canVal)
                                                    <form method="POST" action="{{ route('actions.valider', $a) }}" style="display:inline;"
                                                        onsubmit="return confirm('Valider cette action ?')">
                                                        @csrf
                                            ...
                                            @if($a->statut === 'valide' && in_array(auth()->user()->role, ['admin', 'rbo']))
                                                ...
                                                @endphp
                                                @if($canDev)
                                                    <form method="POST" action="{{ route('actions.devalider', $a) }}"
                                                        style="display:inline;" onsubmit="return confirm('Dévalider cette action ?')">
                                                        @csrf
                                            ...
                                            @if(auth()->user()->role === 'admin')
                                                <form method="POST" action="{{ route('actions.destroy', $a) }}" style="display:inline;"
                                                    onsubmit="return confirm('Supprimer cette action ?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn-ai btn-ai-sm btn-ai-danger" title="Supprimer">
                                                        <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2"
                                                            viewBox="0 0 24 24">
                            ...
                            <tr>
                                <td colspan="6">
                                    <div class="ai-empty">
                                        ...
                                        <p>{{ request('statut') || request('type') || request('compte_id') || request('delegue_id') ? 'Aucun résultat pour les filtres sélectionnés.' : 'Commencez par créer une nouvelle action.' }}
                                        </p>
                                    </div>
                                </td>
                            </tr>
