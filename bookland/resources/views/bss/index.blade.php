...
@section('content')
    <div class="zn-page">
        ...
        <div class="zn-search-bar">
            <form method="GET" action="{{ route('bss.index') }}"
                    ...
                    <div class="zn-filter-group">
                        ...
                        <select name="delegate_id" class="zn-select">
                            ...
                            @foreach($delegates as $del)
                                <option value="{{ $del->id }}" {{ request('delegate_id') == $del->id ? 'selected' : '' }}>
                                    {{ $del->prenom }} {{ $del->nom }}</option>
                            @endforeach
                        </select>
                    </div>
                @endif
                                ...
                                <td>
                                    <div class="actions-cell">
                                        ...
                                        @if(in_array(auth()->user()->role, ['admin', 'rbo']) && $bss->statut === 'soumis')
                                            <form action="{{ route('bss.validate', $bss) }}" method="POST" style="display:inline;">
                                                ...
                                                <button type="submit" class="btn-zn btn-zn-sm btn-zn-primary"
                                                    onclick="return confirm('Valider ce BSS ?')">
                                                    <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"
                                                        viewBox="0 0 24 24">
                                            ...
                                            <button class="btn-zn btn-zn-sm btn-zn-danger"
                                                onclick="openRefuseModal({{ $bss->id }})">
                                                <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"
                                                    viewBox="0 0 24 24">
                                                    <line x1="18" y1="6" x2="6" y2="18" />
                                                    <line x1="6" y1="6" x2="18" y2="18" />
                                        ...
                                        @if(auth()->user()->role === 'admin')
                                            <form action="{{ route('bss.destroy', $bss) }}" method="POST"
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="zn-empty">
                                        ...
                                        <p>{{ request('statut') ? 'Aucun BSS pour les filtres sélectionnés.' : 'Commencez par créer un nouveau BSS.' }}
                                        </p>
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="dlg-modal" role="dialog" aria-modal="true">
            ...
            <div class="dlg-modal-body">
                <form method="POST" id="refuseForm">
                    ...
                    <textarea name="motif_refus" class="textarea-custom" rows="3"
                        placeholder="Expliquez pourquoi ce BSS est refusé..." required></textarea>
