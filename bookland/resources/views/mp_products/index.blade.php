...
<style>
    /* ===== FULL CSS FROM THE DESIGN SYSTEM (same as BSS index) ===== */
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .zn-alert-error {
        ...
        border: 1px solid rgba(232,80,106,.25);
        color: #b83450;
        padding: .85rem 1rem;
        border-radius: var(--r-lg);
        margin-bottom: 1.5rem;
...
<div class="zn-page">
    ...
    <div class="zn-header">
        <div class="zn-header-left">
            <h1>Articles MP</h1>
            <p>Gestion du catalogue (réservé administrateurs).</p>
        </div>
        <div class="zn-header-actions">
            <a href="{{ route('mp-products.create') }}" class="btn-zn btn-zn-primary">
    ...
    <div class="zn-search-bar">
        <form method="GET" action="{{ route('mp-products.index') }}" style="display:flex; gap:1rem; flex-wrap:wrap; align-items:flex-end;">
            <div style="flex:1; min-width:200px;">
                <label class="zn-filter-label" style="display:block; font-size:.7rem; font-weight:600; color:var(--text-secondary); text-transform:uppercase; margin-bottom:.25rem;">Recherche</label>
                <input type="search" name="search" value="{{ request('search') }}" placeholder="Code, nom, éditeur…" class="zn-input">
            </div>
            <div class="filter-actions">
                ...
                <a href="{{ route('mp-products.index') }}" class="btn-zn btn-zn-danger-ghost">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
    ...
    <div class="zn-card">
        <div class="zn-card-header">
            ...
            <span class="zn-count">{{ $products->total() }} article(s)</span>
        </div>
            ...
            <table class="zn-table">
                ...
                <tbody>
                    @forelse($products as $p)
                        <tr>
                            <td><span style="font-family:var(--font-mono); font-size:.78rem;">{{ $p->code_article }}</span></td>
                            <td>{{ $p->editeur }}</td>
                            <td>{{ $p->nom }}</td>
                            <td><span class="dr-badge bd-none" style="background:var(--bg-subtle);">{{ $p->deliveries_count }}</span></td>
                            <td class="actions-cell">
                                <a href="{{ route('mp-products.edit', $p) }}" class="btn-zn btn-zn-sm btn-zn-info" style="background:var(--blue-light); color:var(--blue); border-color:rgba(91,141,238,.2);">
                                ...
                                <form action="{{ route('mp-products.destroy', $p) }}" method="POST" onsubmit="return confirm('Supprimer cet article MP ?');" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <tr>
                            <td colspan="5">
                                <div class="zn-empty">
                                    ...
                                    <h3>Aucun article MP</h3>
                                    <p>{{ request('search') ? 'Aucun article ne correspond à votre recherche.' : 'Commencez par créer un nouvel article.' }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <p style="margin-top: 1.5rem; font-size: .85rem;">
        <a href="{{ route('mp-deliveries.index') }}" style="color: var(--blue); text-decoration: none; display: inline-flex; align-items: center; gap: .3rem;">
            <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
